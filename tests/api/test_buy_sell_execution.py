    ...
    try:
        ...
        for wallet in wallets:
            if (
                ...
                and wallet["type"] == "ACCOUNT_TYPE_CRYPTO"
            ):
                return {
...
def test_buy_order(client, wallet_id, currency="SOL"):
    ...
    try:
        ...
        print(
            f"Minimum order: {min_sol_amount:.6f} {currency} ({min_usdt_amount:.2f} USDT)"
        )
        ...
        if usdt_balance < min_usdt_amount:
            print(
                f"❌ Insufficient USDT balance. Need {min_usdt_amount:.2f} USDT, have {usdt_balance:.2f} USDT"
            )
            return False

        # Place buy order
        print(
            f"Placing buy order for {min_usdt_amount:.2f} USDT worth of {currency}..."
        )
...
def test_sell_order(client, wallet_id, currency="SOL"):
    ...
    try:
        ...
        if crypto_balance < min_sol_amount:
            print(
                f"❌ Insufficient {currency} balance. Need {min_sol_amount}, have {crypto_balance:.6f}"
            )
            return False
...
def main():
    ...
    try:
        ...
        print(
            f"Initial portfolio - Crypto: ${crypto_value:.2f}, Stablecoin: ${stablecoin_value:.2f}"
        )
    except Exception as e:
