...
def create_mock_trader():
    ...
    trader.strat_dct = {"VOLUME-BREAKOUT": []}
...
class TestVolumeBreakoutStrategy(unittest.TestCase):
    ...
    def test_custom_volume_threshold(self):
        ...
        self.assertFalse(
            buy, "Should not execute buy when volume below custom threshold"
        )
        self.assertFalse(
            sell, "Should not execute sell when volume below custom threshold"
        )
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn(
            "VOLUME-BREAKOUT", STRATEGY_REGISTRY, "VOLUME-BREAKOUT not in registry"
        )
        self.assertTrue(
            callable(STRATEGY_REGISTRY["VOLUME-BREAKOUT"]),
            "VOLUME-BREAKOUT not callable",
        )
