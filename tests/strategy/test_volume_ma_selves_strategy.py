...
def create_mock_trader():
    ...
    trader.strat_dct = {"VOLUME-MA-SELVES": []}
...
class TestVolumeMASelvesStrategy(unittest.TestCase):
    ...
    def test_buy_signal_with_high_volume(self):
        ...
        self.assertTrue(
            buy, "Should execute buy when price below tolerance with high volume"
        )
        self.assertFalse(
            sell, "Should not execute sell when price below tolerance with high volume"
        )
    ...
    def test_sell_signal_with_high_volume(self):
        ...
        self.assertFalse(
            buy, "Should not execute buy when price above tolerance with high volume"
        )
        self.assertTrue(
            sell, "Should execute sell when price above tolerance with high volume"
        )
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn(
            "VOLUME-MA-SELVES", STRATEGY_REGISTRY, "VOLUME-MA-SELVES not in registry"
        )
        self.assertTrue(
            callable(STRATEGY_REGISTRY["VOLUME-MA-SELVES"]),
            "VOLUME-MA-SELVES not callable",
        )
