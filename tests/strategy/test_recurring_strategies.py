...
def create_mock_trader():
    ...
    trader.moving_averages = {
        "20": [100.0, 101.0, 102.0, 103.0, 104.0, 105.0]
    }
...
def test_simple_recurring_strategy():
    ...
    buy, sell = strategy_simple_recurring_investment(
        trader2, 100.0, base_date + datetime.timedelta(days=3), 
        investment_interval_days=7
    )
    print(f"    Result: buy={buy}, sell={sell}")
    ...
    buy, sell = strategy_simple_recurring_investment(
        trader3, 100.0, base_date + datetime.timedelta(days=7), 
        investment_interval_days=7
    )
    print(f"    Result: buy={buy}, sell={sell}")
...
def test_weighted_recurring_strategy():
    ...
    buy, sell = strategy_weighted_recurring_investment(
        trader2, "20", 100.0, base_date + datetime.timedelta(days=3), 
        investment_interval_days=7
    )
    print(f"    Result: buy={buy}, sell={sell}")
    assert buy == False, "Investment too soon should not buy"
    ...
    buy, sell = strategy_weighted_recurring_investment(
        trader3, "20", 100.0, base_date + datetime.timedelta(days=7), 
        investment_interval_days=7
    )
    print(f"    Result: buy={buy}, sell={sell}")
    assert buy == True, "Investment after interval should buy"
    ...
    buy, sell = strategy_weighted_recurring_investment(
        trader4, "20", 95.0, base_date + datetime.timedelta(days=14), 
        investment_interval_days=7
    )
    print(f"    Result: buy={buy}, sell={sell}")
...
def test_strategy_registry():
    ...
    assert callable(STRATEGY_REGISTRY["SIMPLE-RECURRING"]), "SIMPLE-RECURRING not callable"
    assert callable(STRATEGY_REGISTRY["WEIGHTED-RECURRING"]), "WEIGHTED-RECURRING not callable"
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 60)
        print("🎉 All tests passed! Recurring investment strategies are working correctly.")
        print("=" * 60)
