...
def create_mock_trader():
    ...
    trader.strat_dct = {"FEAR-GREED-SENTIMENT": []}

    # Mock fear & greed data
    trader.fear_greed_data = [
        {"timestamp": "2024-01-01", "value": 25, "classification": "Fear"},  # Fear
        {"timestamp": "2024-01-02", "value": 75, "classification": "Greed"},  # Greed
    ]
...
class TestFearGreedSentimentStrategy(unittest.TestCase):
    ...
    def test_buy_signal_when_fear_extreme(self):
        ...
        self.assertTrue(
            buy, "Should execute buy when fear & greed index indicates extreme fear"
        )
        self.assertFalse(
            sell,
            "Should not execute sell when fear & greed index indicates extreme fear",
        )
    ...
    def test_buy_signal_when_fear_moderate(self):
        ...
        self.assertTrue(
            buy, "Should execute buy when fear & greed index indicates moderate fear"
        )
        self.assertFalse(
            sell,
            "Should not execute sell when fear & greed index indicates moderate fear",
        )
        self.trader._execute_one_buy.assert_called_once_with(
            "by_percentage", self.new_price
        )
    ...
    def test_sell_signal_when_greed_extreme(self):
        ...
        self.assertFalse(
            buy,
            "Should not execute buy when fear & greed index indicates extreme greed",
        )
        self.assertTrue(
            sell, "Should execute sell when fear & greed index indicates extreme greed"
        )
    ...
    def test_sell_signal_when_greed_moderate(self):
        ...
        self.trader.fear_greed_data = [
            {"timestamp": "2024-01-01", "value": 75, "classification": "Greed"}  # Greed
        ]
        ...
        self.assertFalse(
            buy,
            "Should not execute buy when fear & greed index indicates moderate greed",
        )
        self.assertTrue(
            sell, "Should execute sell when fear & greed index indicates moderate greed"
        )
        self.trader._execute_one_sell.assert_called_once_with(
            "by_percentage", self.new_price
        )
    ...
    def test_no_action_when_neutral_sentiment(self):
        ...
        self.assertFalse(
            buy,
            "Should not execute buy when fear & greed index indicates neutral sentiment",
        )
        self.assertFalse(
            sell,
            "Should not execute sell when fear & greed index indicates neutral sentiment",
        )
        self.trader._record_history.assert_called_with(
    ...
    def test_no_action_when_no_fear_greed_data(self):
        ...
        self.assertFalse(
            buy, "Should not execute buy when no fear & greed data available"
        )
        self.assertFalse(
            sell, "Should not execute sell when no fear & greed data available"
        )
        self.trader._record_history.assert_called_with(
            self.new_price, self.today, NO_ACTION_SIGNAL
        )
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn(
            ...
            STRATEGY_REGISTRY,
            "FEAR-GREED-SENTIMENT not in registry",
        )
        self.assertTrue(
            callable(STRATEGY_REGISTRY["FEAR-GREED-SENTIMENT"]),
            "FEAR-GREED-SENTIMENT not callable",
        )
    ...
    def test_config_integration(self):
        ...
        self.assertIn(
            "FEAR-GREED-SENTIMENT", STRATEGIES, "FEAR-GREED-SENTIMENT not in config"
        )
