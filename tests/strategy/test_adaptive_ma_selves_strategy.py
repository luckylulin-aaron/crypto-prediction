#!/usr/bin/env python3
"""
Test script for ADAPTIVE-MA-SELVES (Adaptive Moving Average) strategy.
"""
...
from app.trading.strategies import strategy_adaptive_ma_selves
...
def create_mock_trader():
    ...
    # Mock moving averages with volatility data
    ...
    trader.price_history = [100.0, 101.0, 102.0, 103.0, 104.0, 105.0, 106.0, 107.0, 108.0, 109.0, 110.0, 111.0, 112.0, 113.0, 114.0, 115.0, 116.0, 117.0, 118.0, 119.0]
    
    return trader


class TestAdaptiveMASelvesStrategy(unittest.TestCase):
    """Test cases for ADAPTIVE-MA-SELVES strategy."""
    ...
    def setUp(self):
        ...
        self.queue_name = "20"
        self.base_tol_pct = 0.1
        self.buy_pct = 0.2
        self.sell_pct = 0.3
        self.volatility_period = 20
        self.volatility_multiplier = 1.0
        
    def test_buy_signal_with_adaptive_tolerance(self):
        """Test buy signal when price is below adaptive tolerance threshold."""
        # Price at 94.5 (10% below MA of 105) with adaptive tolerance
        new_price = 94.5  # (1 - 0.1) * 105 = 94.5
        
        buy, sell = strategy_adaptive_ma_selves(
            self.trader, self.queue_name, new_price, self.today,
            self.base_tol_pct, self.buy_pct, self.sell_pct, 
            self.volatility_period, self.volatility_multiplier
        )
        
        self.assertTrue(buy, "Should execute buy when price below adaptive tolerance")
        self.assertFalse(sell, "Should not execute sell when price below adaptive tolerance")
    ...
    def test_sell_signal_with_adaptive_tolerance(self):
        """Test sell signal when price is above adaptive tolerance threshold."""
        # Price at 115.5 (10% above MA of 105) with adaptive tolerance
        ...
        self.assertFalse(buy, "Should not execute buy when price above adaptive tolerance")
        self.assertTrue(sell, "Should execute sell when price above adaptive tolerance")
    ...
    def test_no_action_when_price_within_tolerance(self):
        """Test no action when price is within adaptive tolerance range."""
        ...
        self.assertFalse(buy, "Should not execute buy when price within adaptive tolerance")
        self.assertFalse(sell, "Should not execute sell when price within adaptive tolerance")
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn("ADAPTIVE-MA-SELVES", STRATEGY_REGISTRY, "ADAPTIVE-MA-SELVES not in registry")
        self.assertTrue(callable(STRATEGY_REGISTRY["ADAPTIVE-MA-SELVES"]), "ADAPTIVE-MA-SELVES not callable")
        
    def test_config_integration(self):
        """Test that strategy is in the config."""
        from app.core.config import STRATEGIES
        
        self.assertIn("ADAPTIVE-MA-SELVES", STRATEGIES, "ADAPTIVE-MA-SELVES not in config")
