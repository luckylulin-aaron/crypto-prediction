...
class TestTrendMASelvesStrategy(unittest.TestCase):
    ...
    def test_buy_signal_with_uptrend(self):
        ...
        buy, sell = strategy_trend_aware_ma_selves(
            ...
            self.tol_pct, self.buy_pct, self.sell_pct, self.trend_period, self.trend_threshold
        )
        
        self.assertTrue(buy, "Should execute buy when price below tolerance in uptrend")
        self.assertFalse(sell, "Should not execute sell when price below tolerance in uptrend")
    ...
    def test_sell_signal_with_downtrend(self):
        ...
        self.assertFalse(buy, "Should not execute buy when price above tolerance in downtrend")
        self.assertTrue(sell, "Should execute sell when price above tolerance in downtrend")
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn("TREND-MA-SELVES", STRATEGY_REGISTRY, "TREND-MA-SELVES not in registry")
        self.assertTrue(callable(STRATEGY_REGISTRY["TREND-MA-SELVES"]), "TREND-MA-SELVES not callable")
