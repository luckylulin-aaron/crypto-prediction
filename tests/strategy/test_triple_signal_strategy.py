...
def create_mock_trader():
    ...
    trader.strat_dct = {"TRIPLE-SIGNAL": []}
...
class TestTripleSignalStrategy(unittest.TestCase):
    ...
    def test_buy_signal_when_two_out_of_three_signals_positive(self):
        ...
        self.assertFalse(
            sell, "Should not execute sell when 2 out of 3 signals are positive"
        )
    ...
    def test_sell_signal_when_two_out_of_three_signals_negative(self):
        ...
        self.assertFalse(
            buy, "Should not execute buy when 2 out of 3 signals are negative"
        )
        self.assertTrue(
            sell, "Should execute sell when 2 out of 3 signals are negative"
        )
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn(
            "TRIPLE-SIGNAL", STRATEGY_REGISTRY, "TRIPLE-SIGNAL not in registry"
        )
        self.assertTrue(
            callable(STRATEGY_REGISTRY["TRIPLE-SIGNAL"]), "TRIPLE-SIGNAL not callable"
        )
