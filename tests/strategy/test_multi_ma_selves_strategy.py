...
def create_mock_trader():
    ...
    trader.strat_dct = {"MULTI-MA-SELVES": []}
...
class TestMultiMASelvesStrategy(unittest.TestCase):
    ...
    def test_buy_signal_with_multiple_confirmations(self):
        ...
        self.assertFalse(
            sell, "Should not execute sell when multiple timeframes confirm buy"
        )
    ...
    def test_sell_signal_with_multiple_confirmations(self):
        ...
        self.assertFalse(
            buy, "Should not execute buy when multiple timeframes confirm sell"
        )
    ...
    def test_no_action_when_insufficient_confirmations(self):
        ...
        self.assertFalse(
            sell, "Should not execute sell when insufficient confirmations"
        )
        self.trader._record_history.assert_called_with(
    ...
    def test_strategy_registry_integration(self):
        ...
        self.assertIn(
            "MULTI-MA-SELVES", STRATEGY_REGISTRY, "MULTI-MA-SELVES not in registry"
        )
        self.assertTrue(
            callable(STRATEGY_REGISTRY["MULTI-MA-SELVES"]),
            "MULTI-MA-SELVES not callable",
        )
