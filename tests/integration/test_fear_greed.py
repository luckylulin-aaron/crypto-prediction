...
def test_fear_greed_client():
    ...
    if current:
        print(
            f"✅ Current Fear & Greed Index: {current['value']} ({current['value_classification']})"
        )
    else:
    ...
    if historical:
        ...
        for i, record in enumerate(historical[:5]):
            print(
                f"     {record['date']}: {record['value']} ({record['value_classification']})"
            )
    else:
    ...
    if historical:
        ...
        if specific:
            print(
                f"✅ Found data for {test_date}: {specific['value']} ({specific['value_classification']})"
            )
        else:
