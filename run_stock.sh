...
LOG_FILE="$BASE_DIR/trading_bot_stock_cron.log"

mkdir -p "$BASE_DIR"
cd "$BASE_DIR"

echo "$(date) - Starting run_stock.sh (cwd=$(pwd))" >> "$LOG_FILE" 2>&1
...
# ---------- RUN THE BOT (STOCK ONLY) ----------
echo "$(date) - Running stock bot..." >> "$LOG_FILE" 2>&1
"$VENV_PYTHON" -u app/core/main.py --asset=stock >> "$LOG_FILE" 2>&1
BOT_EXIT_CODE=$?

if [ $BOT_EXIT_CODE -eq 0 ]; then
    echo "$(date) - Stock bot finished successfully" >> "$LOG_FILE" 2>&1
else
    echo "$(date) - Stock bot exited with code $BOT_EXIT_CODE" >> "$LOG_FILE" 2>&1
fi
