...
def create_price_and_signals_chart(
    ...
    price_df = pd.DataFrame(crypto_prices, columns=['price', 'date', 'open', 'low', 'high'])
    # Handle multiple date formats
...
def create_performance_comparison_chart(
    ...
    # Handle multiple date formats
    df_strategy['date'] = pd.to_datetime(df_strategy['date'], format='mixed', dayfirst=False)
    
    # Baseline performance (buy & hold)
    df_baseline = pd.DataFrame(baseline_prices, columns=['price', 'date'])
    # Handle multiple date formats
    df_baseline['date'] = pd.to_datetime(df_baseline['date'], format='mixed', dayfirst=False)
    
    # Calculate baseline portfolio value (assuming same initial investment)
    initial_investment = df_strategy['portfolio'].iloc[0] if not df_strategy.empty else 1000
    initial_price = df_baseline['price'].iloc[0]
    df_baseline['portfolio'] = (df_baseline['price'] / initial_price) * initial_investment
