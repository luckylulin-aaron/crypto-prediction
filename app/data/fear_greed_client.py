...
class FearGreedClient:
    ...
    def get_historical_fear_greed(self, days: int = 120) -> List[Dict]:
        ...
        try:
            ...
            if cached_data and len(cached_data) >= days:
                logger.info(
                    f"Using cached fear & greed data for {len(cached_data)} days"
                )
            ...
            if data.get("data"):
                ...
                for record in data["data"]:
                    ...
                    record["date"] = datetime.fromtimestamp(timestamp).strftime(
                        "%Y-%m-%d"
                    )
                ...
                logger.info(
                    f"Fetched and cached {len(data['data'])} days of fear & greed data"
                )
                return data["data"]
    ...
    def get_fear_greed_range(self, start_date: str, end_date: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(
                f"Error fetching fear & greed data for range {start_date} to {end_date}: {e}"
            )
            return []
